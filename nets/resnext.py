...
def networksize(resnext_type):
    if resnext_type == 'resnext50':
    ...
    elif resnext_type == 'resnext101':
        network_size = {'Conv2d-1':[64],'Conv2d-2':[192],'Conv2d-3':[384],'Conv2d-4':[256],'Conv2d-5':[256],'fc-1':[4096],'fc-2':[4096]} # for each layer: [number of featuremaps, size of inupt, size of output]
    elif resnext_type == 'resnext152':
        network_size = {'Conv2d-1':[64],'Conv2d-2':[192],'Conv2d-3':[384],'Conv2d-4':[256],'Conv2d-5':[256],'fc-1':[4096],'fc-2':[4096]} # for each layer: [number of featuremaps, size of inupt, size of output]
    return network_size
...
class ResNext(nn.Module):
    ...
    def __init__(self, s_input_channel, n_classes, resnext_type, class_names=100):
        super().__init__()
        block = ResNextBottleNeckC
        if resnext_type == 'resnext50':
            num_blocks = [3, 4, 6, 3]
        elif resnext_type == 'resnext101':
            num_blocks = [3, 4, 6, 23]
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(s_input_channel, 64, 3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
        )
